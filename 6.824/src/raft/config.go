...
func (cfg *config) checkOneLeader() int {
	...
	for iters := 0; iters < 10; iters++ {
		...
		if len(leaders) != 0 {
            fmt.Println("LeaderId:", leaders[lastTermWithLeader][0])
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
    ...
    // fmt.Printf("[nCommitted] cfg.logs:%v index:%v count:%v cmd:%v\n", cfg.logs, index, count, cmd)
	return count, cmd
}
...
func (cfg *config) one(cmd int, expectedServers int) int {
    // fmt.Printf("one cmd:%v expectedServers:%v\n", cmd, expectedServers)
	t0 := time.Now()
	starts := 0
